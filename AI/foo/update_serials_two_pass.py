...
try:
    ...
    print("Processing items with two-pass matching logic...")
    ...
    for index, asset_row in assets_df.iterrows():
        ...
        # --- Pass 1: Location-First Fuzzy Match ---
            ...
            if item_choices_in_loc:
                ...
                if item_score >= 80:
                    ...
                    if pd.notna(purchase_date):
                        ...
                        date_source = f"purchase date (High-Confidence Match in '{best_location_match}')"
                    else:
                        date_source = f"current date (High-Confidence Match in '{best_location_match}', but date invalid)"
                    found_match = True

        # --- Pass 2: Item-Only Fuzzy Match (if Pass 1 failed) ---
        if not found_match:
            best_item_match, item_score = process.extractOne(item_to_find, all_item_choices)
            if item_score >= 85: # Higher threshold for item-only match to avoid false positives
                match_row = register_df[register_df['Item_clean'] == best_item_match].iloc[0]
                purchase_date = match_row['Date of Purchase']
                if pd.notna(purchase_date):
                    year, month = purchase_date.year, purchase_date.month
                    date_source = f"purchase date (Item-Only Match, location='{match_row['Location']}')"
                else:
                    date_source = f"current date (Item-Only Match, but date invalid)"
                found_match = True

        # --- Fallback ---
        if not found_match:
            date_source = "current date (No confident match found)"
